...
For the sake of this exercise, we will show you how to setup a Postgres instance manually via centOS or via AWS RDS.  You may also use your own existing Postgres instance.  You will need to enable logical replication \(shown below\).  
...
#### Now let’s initialize the Postgres server {#now-let%E2%80%99s-initialize-the-postgres-server}
...
[centos@data-adapter-server ~]# sudo npm install ~/Downloads/realm-postgres-adapter-1.10.tgz
```
