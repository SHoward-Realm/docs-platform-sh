...
The WelcomeService displays a webpage at the configured server port that directs the user to download Studio for their various platform. Studio is an electron app that allows for administrative capabilities of the Realm Enterprise Cluster. See [here](../../getting-started/view-data-with-realm-studio.md) for more information:

#### GraphQLService

The GraphQLService allows for web integration into Realm Enterprise Cluster, allowing a REST query to retrieve or manipulate data using the GraphQL query language. It is pulled in by a separate NPM package and is deployed as part of the Core Services app. See [here](../../develop/integration/web-integration.md) for more information.
...
Now let’s set-up your sync worker group, `ssh` to your first consul node. It is assumed that you have followed the instructions to install the Realm prerequisites [here](../../getting-started/install-realm-object-server/manual-install.md#prerequisites)
...
Now we will setup the final piece of our cluster: the CoreServices node. `SSH` to this new node and [install the Realm prerequisites](../../getting-started/install-realm-object-server/manual-install.md#prerequisites) like you did with the sync workers.
